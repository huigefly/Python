...
def share_bt(torrent, save_dir, mode = "download"):
    ...
    ses.async_add_torrent(atp)   # use async add. case: many torrent add. when added, post add_torrent_alert, detail see llibtorrent:seesion.hpp
...
def handle_alert(alert, save_dir):
    ...
    if alert_type == "add_torrent_alert":
        ...
        handle.set_download_limit(300 * 1024)
        handle.set_upload_limit(-1)
    elif alert_type == "state_update_alert":
        download_status = {} 
        if len(alert.status) > 0:
            s = alert.status[0]
            download_status["status"] = STATE_STR[s.state]
            download_status["progress"] = "%.2f" % (s.progress_ppm / 10000)
        ...
        write_fastresume(alert.handle, save_dir, alert.handle.name())
        print("download ok")
        sys.exit(0)
    elif alert_type == "save_resume_data_alert":
        print("save resume data alert")
        write_fastresume(alert.handle, save_dir, alert.handle.name())
    elif alert_type == "metadata_received_alert":
        print("metadata_received_alert")
    elif alert_type == "piece_finished_alert":
        s = alert.handle.status()
        if (int(s.progress * 100) % 2.0) == 0.0:
            # print("piece_finished_alert send save resume alert")
